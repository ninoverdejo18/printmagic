...
interface InkRevealProps {
  ...
  /** Optional background image source to use as the unrevealed mask background */
  bgImageSrc?: string;
...
export default function InkReveal({
  maskColor = [11, 31, 24], // Set default to #0B1F18 color: rgb(11, 31, 24)
  bgImageSrc,
  brushSize = 140,
  lifetime = 750,
  rStart = 15,
  ...
  maxStamps = 300,
  segments = 40,
  wobble = [0.15, 0.08, 0.04],
  gradientInnerRadius = 0.25,
  gradientStops = [1, 0.85, 0],
  className = "",
  style = {},
}: InkRevealProps) {
  ...
  const drawMask = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
    ...
    if (img && img.complete && img.naturalWidth > 0) {
      ...
      // Draw the elegant dark green semi-transparent overlay
      ctx.fillStyle = `rgba(${mc[0]},${mc[1]},${mc[2]}, 0.7)`;
  ...
  }, [carveInk, drawMask, lifetime, rStart]);
